...
MLP Neural Net      & ML        & 64--128--64, 100 ep.             & \texttt{ml\_calibrate.py} \\
...
For all four ML calibration methods, input features are sliding windows of $W=5$ consecutive denoised temperature values. The training set uses the first~80\,\% of the signal; the test set uses the remaining~20\,\%.
...
\textbf{MLP} uses a PyTorch~\cite{pytorch} network $[5\!\rightarrow\!64\!\rightarrow\!128\!\rightarrow\!64\!\rightarrow\!1]$ with ReLU activation, trained for~100 epochs with Adam~\cite{kingma2014adam} at learning rate~$10^{-3}$. Inputs and targets are normalised to~$[0,1]$.
...
VAE               & ML        & bottleneck=3, W=32, 50 ep. & 10.7$\times$         & \texttt{ml\_compress.py} \\
Deep Autoencoder  & ML        & bottleneck=3, W=32, 50 ep. & 10.7$\times$         & \texttt{ml\_compress.py} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{VAE} uses the architecture and training procedure described in Section~\ref{subsec:compression_bg}. The KL weight $\beta=0.001$ is chosen to prevent the KL loss from dominating reconstruction loss during early training. Trained for 50~epochs with Adam~\cite{kingma2014adam}.

\textbf{Deep Autoencoder} uses the four-layer encoder~($32\!\rightarrow\!24\!\rightarrow\!16\!\rightarrow\!8\!\rightarrow\!3$) and mirror decoder. Trained for 50~epochs with Adam~\cite{kingma2014adam} and MSE loss.
